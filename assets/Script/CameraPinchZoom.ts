import { _decorator, Component, Node, EventTouch, Vec3, director, UITransform, view, UIOpacity, tween, v3, Vec2, math } from 'cc';
...
export class CameraPinchZoom extends Component {
    ...
    private clampPosition(targetX: number, targetY: number): Vec3 {
        ...
        // Calculations based on actual screen size and current zoom level
        const boundaryX = Math.max(88.759, (uiTrans.width * this._currentScale - visibleSize.width) / 1);
        const boundaryY = Math.max(88.759, (uiTrans.height * this._currentScale - visibleSize.height) / 0.5);

        // CLAMP WITHOUT HARDCODED NUMBERS (Perfect fit on any screen)
        return new Vec3(
            math.clamp(targetX, -boundaryX, boundaryX), 
            math.clamp(targetY, -boundaryY, boundaryY), 
            0
        );
    }
